...
class rct_net(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.cot1 = CoTAttention(64, 128)
        self.cot2 = CoTAttention(128, 128)
        self.cot3 = CoTAttention(128, 256)
        self.cot4 = CoTAttention(256, 512)
        # self.linear1 = nn.Linear(2048, 8)


        # classifier
        self.classifier = nn.Sequential(
            nn.Conv1d(gmp_dim+cls_dim+de_dims[-1]+1024, 128, 1, bias=bias),
            nn.BatchNorm1d(128),
            nn.Dropout(),
    ...
    def forward(self, x, norm_plt, cls_label):
        ...
        x = torch.cat([x, x_cot, global_context.repeat([1, 1, x.shape[-1]]), cls_token.repeat([1, 1, x.shape[-1]])], dim=1)
        x = self.classifier(x)
...
def rct_net(num_classes=50, **kwargs) -> rct_net:
    return rct_net(num_classes=num_classes, points=1024, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
