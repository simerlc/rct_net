...
class Generate_txt_and_3d_img:
    ...
    def generate_predict_to_txt(self):
        ...
        for batch_id, (points, label, target, norm_plt) in tqdm.tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
            ...
            point_set_without_normal = np.asarray(
                torch.cat([points.permute(0, 2, 1), target[:, :, None]], dim=-1).narrow(0, 1, 1).cpu()).squeeze(0)  # 代标签 没有归一化的点云数据  的numpy形式
            ...
            for n, model, pred_path in zip(self.model_name, self.model, self.all_pred_txt_path):
                ...
                seg_pred = model(points, norm_plt, self.to_categorical(label, 16))
                ...
                seg_pred = np.concatenate([np.asarray(xyz_feature_point.cpu()), seg_pred[:, None, :]],
                                          axis=1).transpose((0, 2, 1))  # 将点云与预测结果进行拼接，准备生成txt文件
                seg_pred = torch.from_numpy(seg_pred)
                seg_pred = seg_pred.narrow(0, 1, 1).squeeze(0).cpu().numpy()
    ...
    def load_cheackpoint_for_models(self, name, model, cheackpoints):
        ...
        for n, m, c in zip(name, model, cheackpoints):
            ...
            from collections import OrderedDict
            state_dict = torch.load(os.path.join('checkpoints/pretrained/rct_net_test', 'best_insiou_model.pth'),
...
if __name__ == '__main__':
    ...
    target_root = r'outputs/test3'  # 输出结果路径
    ...
    # ==============================================
    from model.rct_net import rct_net
    ...
    model1 = rct_net(num_classes=num_classes).eval()
    ...
    if choice_dataset == 'ShapeNet':
        print('实例化ShapeNet')
        TEST_DATASET = PartNormalDataset(npoints=1024, split='test', normalize=False)
        testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=16, shuffle=False, num_workers=12,
                                                     drop_last=False)
        color_map = {idx: i for idx, i in enumerate(np.linspace(0, 0.9, num_classes))}
    else:
        ...
        testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=64, shuffle=False, num_workers=12,
    ...
    model_dict = {
        'rct_net_test': [model1, r'checkpoints/rct_net_test']
    }
