...
class ModelNet10(Dataset):
    def __init__(self, num_points, partition='train'):
        self.data, self.label = load_data10(partition)
        self.num_points = num_points
        self.partition = partition
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(ModelNet10(partition='train', num_points=1024), num_workers=4,
                              batch_size=32, shuffle=True, drop_last=True)
    for batch_idx, (data, label) in enumerate(train_loader):
        print(f"batch_idx: {batch_idx}  | data shape: {data.shape} | ;lable shape: {label.shape}")

    train_set = ModelNet10(partition='train', num_points=1024)
    test_set = ModelNet10(partition='test', num_points=1024)
